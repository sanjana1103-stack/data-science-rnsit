...
const Navbar = () => {
  ...
  return <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 dark:bg-gray-900/90 backdrop-blur-md shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center space-x-8">
          <div className="flex space-x-6">
            {navItems.map(item => item.dropdown ? <div key={item.name} className="relative group">
                  ...
                  <div className="absolute z-10 left-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-left">
                    <div className="py-1 rounded-md bg-white dark:bg-gray-800 shadow-xs">
                      {item.dropdown.map(subItem => <Link key={subItem.name} to={subItem.path} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                          {subItem.name}
                        </Link>)}
                    </div>
                  </div>
                </div> : <Link key={item.name} to={item.path} className="py-2 text-gray-700 dark:text-gray-200 hover:text-rnsit-blue dark:hover:text-rnsit-lightBlue font-medium">
                  {item.name}
                </Link>)}
          </div>
      ...
      {/* Mobile Navigation */}
      {isOpen && <div className="lg:hidden bg-white dark:bg-gray-900 shadow-xl animate-fade-in">
          <div className="container mx-auto px-4 py-4">
            {navItems.map(item => <div key={item.name}>
                {item.dropdown ? <div className="py-2">
                    <div className="font-medium text-gray-700 dark:text-gray-200 mb-2">
                      {item.name}
                    </div>
                    <div className="pl-4 border-l-2 border-gray-200 dark:border-gray-700">
                      {item.dropdown.map(subItem => <Link key={subItem.name} to={subItem.path} onClick={toggleMenu} className="block py-2 text-sm text-gray-600 dark:text-gray-300 hover:text-rnsit-blue dark:hover:text-rnsit-lightBlue">
                          {subItem.name}
                        </Link>)}
                    </div>
                  </div> : <Link to={item.path} onClick={toggleMenu} className="block py-3 text-gray-700 dark:text-gray-200 hover:text-rnsit-blue dark:hover:text-rnsit-lightBlue font-medium border-b border-gray-100 dark:border-gray-800">
                    {item.name}
                  </Link>}
              </div>)}
          </div>
